...
/** @private */
function checkProgress(progress, total, incompat, output, cb) {
    progress++;
    if (progress === total) {
        if (incompat) {
            output.push('');
		...
		dirs.forEach(function (dir) {
			if (dir.indexOf(".") !== 0) {
				...
				fs.access(packageJsonFile, function (err) {
					if (err) {
						console.log(err);
                        progress = checkProgress(progress, total, incompat, output, cb);
					} else {
						fs.readFile(packageJsonFile, function (err, data) {
							if (err) {
								...
								var moduleLicense = modulePkg.license ? ((typeof modulePkg.license === 'string' || modulePkg.license instanceof String) ? modulePkg.license : modulePkg.license.type || null) : null;
